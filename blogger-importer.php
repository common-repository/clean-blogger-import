<?php
/*
Plugin Name: Clean Blogger Importer
Plugin URI: http://wordpress.org/extend/plugins/clean-blogger-import/
Description: blogger, import, clean, no codes
Author: xcakeblogs
...
require_once ABSPATH . 'wp-admin/includes/import.php';
	...
	function xc_strip_attributes($msg, $tag, $attr, $suffix = "")
	{
	     $lengthfirst = 0;
...
class Blogger_Import extends WP_Importer {
	...
	function greet() {
		...
		$title = __('Clean Import Blogger', 'blogger-importer');
...
//echo '<pre>'.print_r($this,1).'</pre>';
	...
	function import_post( $entry ) {
		...
		$post_content = strip_tags($post_content, '<p><a><i><u><b><img>');	
		//Changes </b> and </i> to </strong> and </em>:
		$post_content = str_replace(array('</b>', '</i>'), array('</strong>', '</em>'), $post_content);
		//Changes <b> and <i> to <strong> and <em>
		$post_content = preg_replace(array('`(<b)([^\w])`i', '`(<i)([^\w])`i'), array("<strong$2", "<em$2"), $post_content);
		//Link: Keeps only the href attribute
		$post_content = xc_strip_attributes($post_content, "a", array("href"));

		//Image: Keeps only the src attribute
		$post_content = xc_strip_attributes($post_content, "img", array("src"));
